...
public class MicroscopeConfigManager {
    ...
    /**
     * Get the safe off-slide position for background acquisition or maintenance.
     * This position should be away from the slide to avoid contamination during
     * background image collection or other operations.
     * 
     * @return Array of [x, y, z] coordinates in microns, or null if not configured
     */
    public double[] getSafePosition() {
        logger.debug("Getting safe off-slide position");
        
        Double x = getDouble("stage", "safe_position", "x_um");
        Double y = getDouble("stage", "safe_position", "y_um");
        Double z = getDouble("stage", "safe_position", "z_um");
        
        if (x != null && y != null && z != null) {
            double[] position = new double[]{x, y, z};
            logger.debug("Safe position configured: X={}, Y={}, Z={}", x, y, z);
            
            // Validate that safe position is within stage bounds
            if (isWithinStageBounds(x, y, z)) {
                return position;
            } else {
                logger.error("Safe position ({}, {}, {}) is outside stage bounds!", x, y, z);
                return null;
            }
        }
        
        logger.debug("No safe position configured");
        return null;
    }
    
    /**
     * Get the default safe off-slide position if none is configured.
     * Calculates a position at the center of the stage movement range, raised to a safe Z height.
     * 
     * @return Array of [x, y, z] coordinates in microns, or null if stage limits aren't available
     */
    public double[] getDefaultSafePosition() {
        logger.debug("Calculating default safe position");
        ...
        try {
            ...
            if (xLow == null || xHigh == null || yLow == null || yHigh == null || 
                ...
                logger.error("Cannot calculate default safe position - stage limits not configured");
                return null;
            }
            
            // Calculate center position for X and Y
            double centerX = (xLow + xHigh) / 2.0;
            double centerY = (yLow + yHigh) / 2.0;
            
            // Use a safe Z position (raised above the slide)
            // Use 80% toward the high limit to be safely above any slide
            double safeZ = zLow + (zHigh - zLow) * 0.8;
            
            double[] position = new double[]{centerX, centerY, safeZ};
            logger.info("Calculated default safe position: X={}, Y={}, Z={}", centerX, centerY, safeZ);
            
            return position;
            
        } catch (Exception e) {
            logger.error("Error calculating default safe position", e);
            return null;
        }
    }
    
    /**
     * Get a safe off-slide position, using configured position if available,
     * otherwise calculating a default safe position.
     * 
     * @return Array of [x, y, z] coordinates in microns, or null if unavailable
     */
    public double[] getSafePositionOrDefault() {
        double[] safePos = getSafePosition();
        if (safePos != null) {
            return safePos;
        }
        
        logger.debug("No configured safe position, calculating default");
        return getDefaultSafePosition();
    }
    
    /**
     * Set a new safe position in the runtime configuration.
     * Note: This updates the in-memory configuration but does not persist to file.
     * 
     * For persistence, the YAML file would need to be manually updated to include:
     * <pre>
     * stage:
     *   safe_position:
     *     x_um: [x_value]
     ...
     * @param y Y coordinate in microns  
     * @param z Z coordinate in microns
     * @return true if position is valid and set successfully, false otherwise
     */
    @SuppressWarnings("unchecked")
    public boolean setSafePosition(double x, double y, double z) {
        logger.info("Setting safe position to X={}, Y={}, Z={}", x, y, z);
        
        // Validate position is within stage bounds
        if (!isWithinStageBounds(x, y, z)) {
            logger.error("Cannot set safe position ({}, {}, {}) - outside stage bounds", x, y, z);
            return false;
        }
        
        try {
            // Create safe_position structure in memory
            Map<String, Object> stageSection = getSection("stage");
            if (stageSection == null) {
                logger.error("No stage section found in configuration");
                return false;
            }
            
            // Create or update safe_position sub-section
            Map<String, Object> safePosition = new LinkedHashMap<>();
            safePosition.put("x_um", x);
            safePosition.put("y_um", y);
            safePosition.put("z_um", z);
            
            stageSection.put("safe_position", safePosition);
            
            logger.info("Safe position set in runtime configuration (not persisted to file)");
            logger.warn("To persist this safe position, manually add to YAML config:\n" +
                    "stage:\n" +
                    "  safe_position:\n" +
                    "    x_um: {}\n" +
                    "    y_um: {}\n" +
            ...
            logger.error("Error setting safe position", e);
            return false;
        }
    }
    
    /**
     * Generate YAML snippet for adding safe position to configuration file.
     * This provides the exact text that should be added to the stage section.
     * 
     * @param x X coordinate in microns
     * @param y Y coordinate in microns
     * @param z Z coordinate in microns
     * @return YAML snippet string
     */
    public String generateSafePositionYAML(double x, double y, double z) {
        return String.format(
                "# Add this to the stage section of your configuration file:\n" +
                "stage:\n" +
                "  # ... existing stage configuration ...\n" +
                "  safe_position:\n" +
                "    x_um: %.1f\n" +
                "    y_um: %.1f\n" +
                "    z_um: %.1f\n" +
                "    # Safe off-slide position for background acquisition and maintenance",
                x, y, z);
    }
    
    /**
     * Move the microscope stage to the configured safe position.
     * This is useful for background acquisition, maintenance, or any operation requiring
     * the stage to be positioned away from the slide.
     * 
     * @return true if movement was successful, false if no safe position is available or movement failed
     * @throws Exception if stage movement fails
     */
    public boolean moveToSafePosition() throws Exception {
        logger.info("Moving stage to safe position");
        
        double[] safePos = getSafePositionOrDefault();
        if (safePos == null) {
            logger.error("Cannot move to safe position - no safe position configured or calculable");
            return false;
        }
        ...
        logger.info("Moving stage to safe position: X={}, Y={}, Z={}", x, y, z);
        
        try {
            // Import MicroscopeController dynamically to avoid circular dependencies
            Class<?> controllerClass = Class.forName("qupath.ext.qpsc.controller.MicroscopeController");
            Object controllerInstance = controllerClass.getMethod("getInstance").invoke(null);
            
            // Move XY first, then Z for safety
            controllerClass.getMethod("moveStageXY", double.class, double.class).invoke(controllerInstance, x, y);
            logger.debug("XY movement to safe position completed");
            
            controllerClass.getMethod("moveStageZ", double.class).invoke(controllerInstance, z);
            logger.debug("Z movement to safe position completed");
            
            logger.info("Successfully moved stage to safe position: X={}, Y={}, Z={}", x, y, z);
            return true;
            
        } catch (Exception e) {
            logger.error("Failed to move stage to safe position", e);
            throw new Exception("Stage movement to safe position failed: " + e.getMessage(), e);
        }
    }
    ...
    public List<String> validateConfiguration() {
        ...
        // Check for safe position (recommended but not required)
        double[] safePos = getSafePosition();
        if (safePos == null) {
            logger.warn("No safe position configured - consider adding stage.safe_position for background acquisition");
            // Don't add to missing list since this is optional
        } else {
            logger.info("Safe position configured: X={}, Y={}, Z={}", safePos[0], safePos[1], safePos[2]);
        }
